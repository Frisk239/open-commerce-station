...
export default async function OrderPage({ params }: { readonly params: Promise<{ number: string }> }) { const email = (await shopperAuth())?.user?.email; if (!email) redirect("/account/login?returnTo=/account/orders"); const order = await readShopperOrder("cn", email, (await params).number); if (!order) notFound(); return <ShopperOrderDetail order={order} />; }
