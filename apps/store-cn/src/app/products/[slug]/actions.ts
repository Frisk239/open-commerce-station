...
export async function addToCart(_state: CartActionState, formData: FormData): Promise<CartActionState> {
  ...
  const token = cookieStore.get("ocs_cart_cn")?.value ?? randomUUID();
  try {
    const result = await addVariantToCart("cn", token, variantId, quantity);
    cookieStore.set("ocs_cart_cn", token, { httpOnly: true, sameSite: "lax", secure: process.env.NODE_ENV === "production", path: "/", maxAge: 60 * 60 * 24 * 30 });
