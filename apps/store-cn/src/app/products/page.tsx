...
export default async function CatalogPage({ searchParams }: { readonly searchParams: Promise<{ q?: string; group?: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-stone-50 text-stone-950">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-stone-200 px-5 py-4 md:px-10"><Link href="/" className="font-bold">{identity.name}</Link><nav aria-label="店面导航" className="flex gap-5 text-sm text-stone-600"><Link href="/products">全部商品</Link><Link href="/cart">购物车</Link><Link href="/account">账户</Link></nav></header>
      <section className="mx-auto max-w-7xl px-5 py-12 md:px-10 md:py-16">
        <div className="flex flex-wrap items-end justify-between gap-5"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-800">全部商品</p><h1 className="mt-3 font-[family-name:var(--font-display)] text-5xl font-semibold tracking-[-0.04em]">全部商品</h1></div><p className="text-sm text-stone-500">{products.length} 件商品</p></div>
