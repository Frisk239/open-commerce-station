...
export default async function StartPaypalPage({ params }: { readonly params: Promise<{ id: string }> }) {
  const email = (await shopperAuth())?.user?.email;
  const { id } = await params;
  if (!email) redirect(`/account/login?returnTo=${encodeURIComponent(`/checkout/paypal/start/${id}`)}`);
