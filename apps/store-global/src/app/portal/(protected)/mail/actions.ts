...
export async function retryMail(formData: FormData): Promise<void> {
  ...
  if (typeof id === "string") {
    await retryNoticeMail("global", id).catch(() => undefined);
    if (mailTransportMode() === "deterministic") {
      await drainNoticeMails("global", async (mail) => new DeterministicMailTransport().send(mail));
    } else {
      const config = await readMailTransportConfig("global");
      if (config) await drainNoticeMails("global", async (mail) => createNodemailerTransport(config).send(mail));
    }
  }
  revalidatePath("/portal/mail");
