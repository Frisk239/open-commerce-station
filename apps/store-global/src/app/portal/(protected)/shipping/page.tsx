...
export default async function ShippingPage({ searchParams }: { readonly searchParams: Promise<{ saved?: string; error?: string }> }) { const [rates, params] = await Promise.all([listShippingRates("global"), searchParams]); return <ShippingManager flavor="global" rates={rates} saveAction={saveShipping} deleteAction={removeShipping} saved={params.saved === "1"} error={params.error === "1"} />; }
