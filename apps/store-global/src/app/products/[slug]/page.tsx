...
export default async function ProductPage({ params }: { readonly params: Promise<{ slug: string }> }) {
  ...
  const [identity, product] = await Promise.all([getStoreIdentity(), readPublishedProduct("global", slug)]);
  if (!product) notFound();
  const name = pickLocalizedText(product.name, "en", "en");
  return (
    <main className="min-h-screen bg-stone-50 text-stone-950">
      <header className="flex items-center justify-between border-b border-stone-200 px-5 py-4 md:px-10"><Link href="/" className="font-bold">{identity.name}</Link><nav className="flex gap-5 text-sm text-stone-600"><Link href="/products">Back to all products</Link><Link href="/cart">Cart</Link></nav></header>
      <article className="mx-auto grid max-w-7xl gap-10 px-5 py-10 md:px-10 lg:grid-cols-[minmax(0,1.35fr)_minmax(22rem,0.65fr)] lg:py-16">
        ...
        <div className="h-fit lg:sticky lg:top-8"><p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-800">Product</p><h1 className="mt-3 font-[family-name:var(--font-display)] text-4xl font-semibold tracking-[-0.04em] md:text-5xl">{name}</h1><div className="mt-7"><ProductPurchasePanel options={product.options} variants={product.variants} locale="en" currency="USD" action={addToCart} /></div>{pickLocalizedText(product.story, "en", "en") ? <div className="mt-9 border-t border-stone-200 pt-7"><h2 className="text-sm font-bold">Product story</h2><p className="mt-3 whitespace-pre-line leading-7 text-stone-600">{pickLocalizedText(product.story, "en", "en")}</p></div> : null}</div>
      </article>
    </main>
  );
}
