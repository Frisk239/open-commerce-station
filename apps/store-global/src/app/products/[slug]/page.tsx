...
export default async function ProductPage({ params }: { readonly params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-stone-50 text-stone-950">
      <header className="flex items-center justify-between border-b border-stone-200 px-5 py-4 md:px-10"><Link href="/" className="font-bold">{identity.name}</Link><Link href="/products" className="text-sm text-stone-600">Back to all products</Link></header>
