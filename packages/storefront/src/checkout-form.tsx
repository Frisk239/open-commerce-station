...
import type { CheckoutAddress, CheckoutQuote, CurrencyCode, StationFlavor } from "@ocs/core";
...
export interface CheckoutActionState {
  ...
  readonly error?: "empty-cart" | "invalid-cart" | "invalid-address" | "invalid-discount" | "no-shipping-rate" | "invalid-shipping-rate" | "unexpected";
}
...
function QuoteButton({ label }: { readonly label: string }) {
  const { pending } = useFormStatus();
  return <button disabled={pending} className="rounded-full bg-stone-950 px-6 py-3 text-sm font-bold text-white disabled:cursor-wait disabled:opacity-60">{pending ? `${label}…` : label}</button>;
}

export function CheckoutForm({ flavor, currency, email, storeName, logoUrl, initialFields, action }: CheckoutFormProps) {
